import {
  Accessory,
  AdaptiveLightningController,
  AdaptiveLightningControllerMode,
  Categories,
  Characteristic,
  ColorUtils,
  Service,
...
/**
 * This example light gives an example how a light with AdaptiveLightning (in AUTOMATIC mode) support
 * can look like.
 ...
 *
 * AdaptiveLightning setup is pretty much at the end of the file, don't miss it.
 */

const lightUUID = uuid.generate('hap-nodejs:accessories:light-adaptive-lightning');
...
accessory.getService(Service.AccessoryInformation)!
  ...
  .setCharacteristic(Characteristic.Model, "Light with AdaptiveLightning")
...
lightbulbService.getCharacteristic(Characteristic.Brightness) // Brightness characteristic is required for adaptive lightning
  .onGet(() => {
...
lightbulbService.getCharacteristic(Characteristic.ColorTemperature) // ColorTemperature characteristic is required for adaptive lightning
  .onGet(() => {
...
const adaptiveLightningController = new AdaptiveLightningController(lightbulbService, {
  ...
  controllerMode: AdaptiveLightningControllerMode.AUTOMATIC,
});
accessory.configureController(adaptiveLightningController);
