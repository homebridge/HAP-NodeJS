...
const defaultPlist: string = resolve(simulator, 'Contents/Frameworks/HAPAccessoryKit.framework/Resources/default.metadata.plist')
const defaultMfiPlist: string = resolve(simulator, 'Contents/Frameworks/HAPAccessoryKit.framework/Resources/default_mfi.metadata.plist')
...
export interface GeneratedCharacteristic {
  ...
  adminOnlyAccess?: Characteristic.Access[]
...
const plistData = readPlistFileSync(metadataFile) as any
const simulatorPlistData = readPlistFileSync(defaultPlist) as any
const simulatorMfiPlistData = existsSync(defaultMfiPlist) ? readPlistFileSync(defaultMfiPlist) : undefined as any
...
for (const [id, definition] of Object.entries(characteristics)) {
  try {
    ...
    let validBitMasks: Record<string, string> | undefined
    if (validBits) {
      validBitMasks = {}
