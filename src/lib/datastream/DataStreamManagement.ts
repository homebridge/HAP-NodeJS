...
import { decode, encode, writeUInt16 } from '../util/tlv.js'
...
export class DataStreamManagement {
  ...
  private handleSetupDataStreamTransportWrite(value: any, callback: CharacteristicSetCallback, connection: HAPConnection) {
    ...
    if (sessionCommandType === SessionCommandType.START_SESSION) {
      ...
      this.dataStreamServer.prepareSession(connection, controllerKeySalt, (error, preparedSession) => {
        ...
        const listeningPort = encode(TransportSessionConfiguration.TCP_LISTENING_PORT, writeUInt16(preparedSession.port!))

        let response: Buffer = Buffer.concat([
          encode(SetupDataStreamWriteResponseTypes.STATUS, DataStreamStatus.SUCCESS),
          encode(SetupDataStreamWriteResponseTypes.TRANSPORT_TYPE_SESSION_PARAMETERS, listeningPort),
        ])
        this.lastSetupDataStreamTransportResponse = response.toString('base64') // save last response without accessory key salt

        response = Buffer.concat([
          response,
          encode(SetupDataStreamWriteResponseTypes.ACCESSORY_KEY_SALT, preparedSession.accessoryKeySalt),
        ])
  ...
  private buildSupportedDataStreamTransportConfigurationTLV(supportedConfiguration: TransportType[]): string {
    ...
    supportedConfiguration.forEach((type) => {
      const transportType = encode(TransportTypeTypes.TRANSPORT_TYPE, type)
      const transferTransportConfiguration = encode(TransferTransportConfigurationTypes.TRANSFER_TRANSPORT_CONFIGURATION, transportType)
