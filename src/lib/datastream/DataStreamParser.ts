...
import { isValid, unparse, write } from '../util/uuid.js'
...
export class UUID extends ValueWrapper<string> {
  constructor(value: string) {
    assert(isValid(value), 'invalid uuid format')
    super(value)
  }
}
...
export class DataStreamParser {
  public static decode(buffer: DataStreamReader): any {
    ...
    if (tag === DataFormatTags.INVALID) {
      ...
      let element = this.decode(buffer)
      while (element !== Magics.TERMINATOR) {
        array.push(element)
        element = this.decode(buffer)
      }
      ...
      let key = this.decode(buffer)
      while (key !== Magics.TERMINATOR) {
        dictionary[key] = this.decode(buffer) // decode value
        key = this.decode(buffer)
      }
      ...
      case 'object':
        if (Array.isArray(data)) {
          ...
          if (data instanceof Int8) {
            ...
            throw new TypeError(`Unknown wrapped object 'ValueWrapper' of class ${data.constructor.name}`)
          }
...
export class DataStreamReader {
  ...
  readUUID(): string { // big endian
    ...
    const value = unparse(this.data, this.readerIndex)
    this.readerIndex += 16
    return this.trackData(value)
  }
}
...
export class DataStreamWriter {
  ...
  private writeLength64LE(length: number): void {
    this.ensureLength(8)
    writeUInt64LE(length, this.data, this.writerIndex)
    this.writerIndex += 8
  }
  ...
  writeUUID(uuid_string: string): void {
    assert(isValid(uuid_string), 'supplied uuid is invalid')
    ...
    write(uuid_string, this.data, this.writerIndex)
    this.writerIndex += 16
  }
}
