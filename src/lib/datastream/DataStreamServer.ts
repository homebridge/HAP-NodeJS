 ...
 */
export interface PreparedDataStreamSession {
...
export class DataStreamServer extends EventEmitter {
  ...
  private handleSessionIdentification(connection: DataStreamConnection, firstFrame: HDSFrame, callback: IdentificationCallback) {
    let identifiedSession: PreparedDataStreamSession | undefined
...
export class DataStreamConnection extends EventEmitter {
    ...
    if (this.state === ConnectionState.UNIDENTIFIED) {
      ...
      this.emit(DataStreamConnectionEvent.IDENTIFICATION, firstFrame, (identifiedSession?: PreparedDataStreamSession) => {
        if (identifiedSession) {
          ...
          debug('[%s] Registering CLOSED handler to HAP connection. Connection currently has %d close handlers!', this.remoteAddress, this.connection.listeners(HAPConnectionEvent.CLOSED).length)
        }
      })
    ...
    messages.forEach((message) => {
      if (message.type === MessageType.RESPONSE) {
        ...
        if (message.type === MessageType.EVENT) {
          ...
          const eventHandler: EventHandler = handler.eventHandler[message.topic]
          ...
          const requestHandler: RequestHandler = handler.requestHandler[message.topic]
  ...
  private decodePayloads(frames: HDSFrame[]) {
    ...
    frames.forEach((frame) => {
      ...
      let status: HDSStatus | undefined
