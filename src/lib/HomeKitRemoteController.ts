...
export class HomeKitRemoteController {
  ...
  _buttonEvent = (buttonID: number, buttonState: ButtonState, timestamp: number, activeIdentifier: number) => {
    if (activeIdentifier === 0) return; // cannot press button if no device is selected
    ...
    const activeIdentifierTlv = tlv.encode(ButtonEvent.ACTIVE_IDENTIFIER, tlv.writeUInt32(activeIdentifier));
  ...
  _handleTargetControlWrite = (value: any, callback: CharacteristicSetCallback) => {
    ...
    debug(
      "Received TargetControl write operation %s%s",
      Operation[operation],
      targetConfiguration !== undefined ? "with configuration: " + JSON.stringify(targetConfiguration) : "",
    );

    if (
      targetConfiguration === undefined &&
      (operation === Operation.ADD || operation === Operation.UPDATE || operation === Operation.REMOVE)
    ) {
    ...
    switch (operation) {
      ...
      case Operation.UPDATE:
        ...
        if (targetConfiguration!.targetName) savedTargetConfiguration.targetName = targetConfiguration!.targetName;
    ...
    if (operation === Operation.ADD) this.setActiveIdentifier(targetConfiguration!.targetIdentifier);
  };
