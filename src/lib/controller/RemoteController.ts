...
export class RemoteController extends EventEmitter
  ...
  private handleTargetControlWrite(value: any, callback: CharacteristicSetCallback): void {
    ...
    let targetConfiguration: TargetConfiguration | undefined
  ...
  private sendButtonEvent(button: ButtonType, buttonState: ButtonState) {
    ...
    const buttonIdTlv = encode(
      ButtonEvent.BUTTON_ID,
      buttonID,
    )

    const buttonStateTlv = encode(
      ButtonEvent.BUTTON_STATE,
      buttonState,
    )

    const timestampTlv = encode(
      ButtonEvent.TIMESTAMP,
      writeVariableUIntLE(new Date().getTime()),
    ...
    const activeIdentifierTlv = encode(
      ButtonEvent.ACTIVE_IDENTIFIER,
  ...
  private parseTargetConfigurationTLV(data: Buffer): TargetConfiguration {
    ...
    const identifier = readUInt32(configTLV[TargetConfigurationTypes.TARGET_IDENTIFIER])
    ...
    if (configTLV[TargetConfigurationTypes.TARGET_CATEGORY]) {
      category = readUInt16(configTLV[TargetConfigurationTypes.TARGET_CATEGORY])
    }

    const buttonConfiguration: Record<number, ButtonConfiguration> = {}

    if (configTLV[TargetConfigurationTypes.BUTTON_CONFIGURATION]) {
      const buttonConfigurationTLV = decodeList(configTLV[TargetConfigurationTypes.BUTTON_CONFIGURATION], ButtonConfigurationTypes.BUTTON_ID)
      buttonConfigurationTLV.forEach((entry) => {
        const buttonId = entry[ButtonConfigurationTypes.BUTTON_ID][0]
        const buttonType = readUInt16(entry[ButtonConfigurationTypes.BUTTON_TYPE])
        let buttonName
  ...
  private updatedTargetConfiguration(): void {
    ...
    for (const configuration of Object.values(this.targetConfigurations)) {
      ...
      for (const value of configuration.buttonConfiguration.values()) {
        ...
        if (value.buttonName) {
          tlvBuffer = Buffer.concat([
            tlvBuffer,
            encode(
              ButtonConfigurationTypes.BUTTON_NAME,
              value.buttonName,
            ),
          ])
        }
      ...
      const buttonConfiguration = encode(
      ...
      bufferList.push(encode(TargetControlList.TARGET_CONFIGURATION, targetConfiguration))
    }
  ...
  private buildTargetControlSupportedConfigurationTLV(configuration: SupportedConfiguration): string {
    const maximumTargets = encode(
      TargetControlCommands.MAXIMUM_TARGETS,
      configuration.maximumTargets,
    )

    const ticksPerSecond = encode(
      TargetControlCommands.TICKS_PER_SECOND,
      writeVariableUIntLE(configuration.ticksPerSecond),
    )

    const supportedButtonConfigurationBuffers: Uint8Array[] = []
    configuration.supportedButtonConfiguration.forEach((value) => {
      const tlvBuffer = encode(
    ...
    const supportedButtonConfiguration = encode(
      TargetControlCommands.SUPPORTED_BUTTON_CONFIGURATION,
      Buffer.concat(supportedButtonConfigurationBuffers),
    )

    const type = encode(TargetControlCommands.TYPE, configuration.hardwareImplemented ? 1 : 0)
  ...
  private handleSelectedAudioConfigurationWrite(value: any, callback: CharacteristicSetCallback): void {
    ...
    const selectedAudioStreamConfiguration = decode(
    ...
    const parameters = decode(selectedAudioStreamConfiguration[AudioCodecConfigurationTypes.CODEC_PARAMETERS])
  ...
  private static buildSupportedAudioConfigurationTLV(configuration: SupportedAudioStreamConfiguration): string {
    ...
    const supportedAudioStreamConfiguration = encode(
  ...
  private static buildCodecConfigurationTLV(codecConfiguration: AudioCodecConfiguration): Buffer {
    ...
    if (parameters.rtpTime) {
      parametersTLV = Buffer.concat([
        parametersTLV,
        encode(AudioCodecParametersTypes.PACKET_TIME, parameters.rtpTime),
      ])
    }

    return encode(
      AudioCodecConfigurationTypes.CODEC_TYPE,
      codecConfiguration.codecType,
