...
export function encode(type: number, data: Buffer | number | string, ...args: any[]) {
  ...
  if (typeof data === "number") data = bufferShim.from([data]) as Buffer;
  else if (typeof data === "string") data = bufferShim.from(data) as Buffer;
  ...
  if (data.length <= 255) {
    ...
    for (; leftLength > 0; ) {
      if (leftLength >= 255) {
        ...
        tempBuffer = Buffer.concat([
          tempBuffer,
          bufferShim.from([type, leftLength]),
