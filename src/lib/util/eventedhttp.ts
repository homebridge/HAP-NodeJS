...
export class Session extends EventEmitter<HAPSessionEventMap> {
  ...
  /**
   * Session dictionary indexed by sessionID.
   * SessionID is an ID generated by HAP-NodeJS unique to every open connection (it's based on the address and port).
   * SessionID gets passed to get/set handlers for characteristics. We mainly need this dictionary in order to access
   * the sharedSecret in the HAPEncryption object from the SetupDataStreamTransport characteristic set handler.
   */
