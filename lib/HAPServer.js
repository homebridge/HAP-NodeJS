...
// M5-2
HAPServer.prototype._handlePairStepFour = async function(request, response, session, clientUsername, clientLTPK, clientProof, hkdfEncKey) {
  ...
  await sodium.ready;
  if (!sodium.crypto_sign_verify_detached(clientProof, completeData, clientLTPK)) {
...
HAPServer.prototype._handlePairStepFive = function(request, response, session, clientUsername, clientLTPK, hkdfEncKey) {
  ...
  var serverProof = sodium.crypto_sign_detached(material, privateKey);
...
HAPServer.prototype._handlePairVerifyStepOne = async function(request, response, session, objects) {
  ...
  var keyPair = await encryption.generateCurve25519KeyPair();
  var secretKey = bufferShim.from(keyPair.privateKey);
  var publicKey = bufferShim.from(keyPair.publicKey);
  var sharedSec = bufferShim.from(await encryption.generateCurve25519SharedSecKey(keyPair.privateKey, clientPublicKey));
...
HAPServer.prototype._handlePairVerifyStepTwo = function(request, response, session, events, objects) {
  ...
  if (!sodium.crypto_sign_verify_detached(proof, material, clientPublicKey)) {
...
HAPServer.prototype._handleRemotePairVerifyStepOne = async function(request, remoteSession, session, objects) {
