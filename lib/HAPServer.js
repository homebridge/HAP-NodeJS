...
// Hue Api starts here
var hueLights = require('./util/hue').lights;
var hueGroups = require('./util/hue').groups;
var hueLight = require('./util/hue').light;
var hueState = require('./util/hue').state;
var hueDescription = require('./util/ssdp').description;
var ssdp = require('./util/ssdp');
...
HAPServer.handlers = {
    ...
    '/api/.*/lights': '_handleHueLights',
    '/api/.*/groups/0': '_handleHueGroups',
    '/api/.*/lights/.*/state': '_handleHueLightState',
    '/api/.*/lights/.*': '_handleHueLight',
    '/description.xml': '_handleSlash'
}
...
HAPServer.prototype._onListening = function(port) {
    ...
    this._ssdpServer = ssdp.startSsdpServer(port);
}
...
// call out for hue API
// operations to implement
// /api/{name}/lights
// /api/{name}/groups/{group}
// /api/{name}/lights/{bulb}


HAPServer.prototype._handleHueLights = function(request, response, session, events, requestData) {
    //  if (!this.allowInsecureRequest && !session.encryption) {
    ...
    this.emit('accessories', once(function(err, accessories) {
        ...
        response.end(JSON.stringify(hueLights(accessories)));
    }));
}

HAPServer.prototype._handleHueLight = function(request, response, session, events, requestData) {

//  var data = hueLight(request.url,JSON.parse(requestData.toString()));

  var aid = parseInt(request.url.split("/")[4]);
  var iid = 9;
  var data=[];
  ...
  this.emit('get-characteristics', data, events, once(function(err, characteristics) {
      ...
      response.end(JSON.stringify(hueLight(characteristics)));

  }.bind(this)), false, session.sessionID);
}

HAPServer.prototype._handleHueLightState = function(request, response, session, events, requestData) {

    var data = hueState(request.url,JSON.parse(requestData.toString()));
...
HAPServer.prototype._handleHueGroups = function(request, response, session, events, requestData) {
    // call out to listeners to retrieve the latest accessories JSON
    response.writeHead(200, {
        "Content-Type": "application/json"
    });
    response.end(hueGroups());
}

HAPServer.prototype._handleSlash = function(request, response, session, events, requestData) {

    response.writeHead(200, {
        "Content-Type": "application/json"
    });
    response.end(hueDescription());
}
