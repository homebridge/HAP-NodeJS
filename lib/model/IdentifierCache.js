...
IdentifierCache.prototype.getAID = function(accessoryUUID) {
  ...
  this.getCache('|nextIID');

  return this.getCache(key) || this.setCache(key, this.getNextAID());
}

IdentifierCache.prototype.getIID = function(accessoryUUID, serviceUUID, characteristicUUID) {
  var key = accessoryUUID + '|' + serviceUUID + (characteristicUUID ? '|' + characteristicUUID : '');
...
IdentifierCache.prototype.getNextIID = function(accessoryUUID) {
  ...
  var nextIID = this.getCache(key) || 1;
