...
function load(dir) {
  ...
  return accessories.map(function(accessory) {
    return (accessory instanceof Accessory) ? accessory : importAccessory(accessory);
  })
}
...
function importAccessory(json) {
  ...
  json.services.forEach(function(serviceJSON) {
    var service = importService(serviceJSON);
...
function importService(json) {
  ...
  json.characteristics.forEach(function(characteristicJSON) {
    var characteristic = importCharacteristic(characteristicJSON);
...
function importCharacteristic(json) {
