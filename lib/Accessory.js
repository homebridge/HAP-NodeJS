...
Accessory.prototype._handleSetCharacteristics = function(data, events, callback, remote, connectionID) {
  ...
  data.forEach(function(characteristicData) {
    ...
    // JSP - Hack to work around current iOS bug where MotionDetections are being unregistered
        if (characteristic.displayName == "Motion Detected" && ev === false) {
          debug('[%s] **** Ignoring Unresigster Request for "Motion Detected"', this.displayName);
          // set ev to undefined
          ev = undefined;
        }
