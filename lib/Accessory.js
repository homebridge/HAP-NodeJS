...
var AdvertiserAvahi = require('./AdvertiserAvahi').Advertiser;
var Advertiser;
try {
  Advertiser = require('./Advertiser').Advertiser;
} catch(err) {
  Advertiser = null;
}
...
function Accessory(displayName, UUID) {
  ...
  this
    ...
    .setCharacteristic(Characteristic.Manufacturer, process.env.HOMEKIT_MANUFACTURER || "Default-Manufacturer")
    .setCharacteristic(Characteristic.Model, process.env.HOMEKIT_MODEL || "Default-Model")
    .setCharacteristic(Characteristic.SerialNumber, process.env.HOMEKIT_SERIAL || "Default-SerialNumber");
...
Accessory.prototype.removeAllBridgedAccessories = function() {
  for (var i = this.bridgedAccessories.length - 1; i >= 0; i --) {
    console.log('remove', this.bridgedAccessories[i]);
...
Accessory.prototype.publish = function(info, allowInsecureRequest) {
  ...
  if (Config.useAvahi()) {
    this._advertiser = new AdvertiserAvahi(this._accessoryInfo, Config.getAvahiFilepath());
  } else {
    if (Advertiser) {
      this._advertiser = new Advertiser(this._accessoryInfo);
    } else {
      throw new Error('mdns components not installed! make sure to set env variable "AVAHI_TARGET_FILE" to create avahi config file');
    }
  }
