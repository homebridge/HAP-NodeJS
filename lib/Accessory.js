...
Accessory.prototype._handleGetCharacteristic = function(aid, iid, events, callback) {
  ...
  characteristic.getValue(function(err, value) {
    ...
    debug('[%s] Got Characteristic "%s" value: ', this.displayName, characteristic.displayName, value);

    if (err) {
      debug('[%s] Error getting value for Characteristic "%s": ', this.displayName, characteristic.displayName, err.message);
      callback(err);
    }
