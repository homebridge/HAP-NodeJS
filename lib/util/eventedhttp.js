...
EventedHTTPServer.prototype.listen = function(port) {
...
function EventedHTTPServerConnection(clientSocket) {
  ...
  this._pendingEventData = Buffer(0); // event data waiting to be sent until after an in-progress HTTP response is being written
...
EventedHTTPServerConnection.prototype.sendEvent = function(event, data, contentType, excludeEvents) {
  ...
  var linebreak = new Buffer("0D0A","hex");
  data = Buffer.concat([
    new Buffer('EVENT/1.0 200 OK'), linebreak,
    new Buffer('Content-Type: ' + contentType), linebreak,
    new Buffer('Content-Length: ' + data.length), linebreak,
    linebreak,
    data
  ]);
...
EventedHTTPServerConnection.prototype._sendPendingEvents = function() {
  ...
  // clear the buffer
  this._pendingEventData = new Buffer(0);
}
...
EventedHTTPServerConnection.prototype._onHttpServerRequest = function(request, response) {
  ...
  response.on('finish', function() {
    ...
    debug("[%s] HTTP Repsonse is finished", this._remoteAddress);
