...
function handleHueApi(that, request, response, session, events, requestData) {
    ...
    // /api/{username}/lights/# - returns On/Off status of a light
    ...
    switch (Object.getOwnPropertyNames(requestData)[0]) {
        ...
        case "bri":
            characteristic = [{
                ...
                "value": parseInt(requestData.bri / 2.55+.5)
            }];
            break;
        default:
...
function _parseHbtoHue(accessories) {
    ...
    for (var accessory in accessories) {
        ...
        //        debug( "Device Detail" ,JSON.stringify(device, null, 2));
        ...
        for (var service in device.services) {
            ...
            if (serviceType.startsWith("00000043") || serviceType.startsWith("00000047") ||
                ...
                for (var id in device.services[service].characteristics) {
                    ...
                    // Brightness characteristic || RotationSpeed
                    if (cType.startsWith("00000008") || cType.startsWith("00000029")) {
                ...
                if (actions.length > 0) {
                    ...
                    debug('device response', count, hueDevices[count]);
                }
            }
        }
    ...
    debug("HUE response",JSON.stringify(hueDevices, null, 2));
    return hueDevices;
}
...
function _getBiid(lightid) {
    ...
    if (!hueDevices[lightid]) {
        console.log("ERROR: Alexa missing device, please discover devices again.", lightid);
        return (0);
    } else {
        var mac = hueDevices[lightid].uniqueid.split(':');
        var iid = mac[7].split('-');
