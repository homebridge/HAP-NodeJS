...
def HKDF(SKM, dkLen, XTS=None, CTXinfo="", digest=sha256,
    ...
    print "salt " + ":".join("{0:x}".format(ord(c)) for c in XTS)
    print "ikm " + ":".join("{0:x}".format(ord(c)) for c in SKM)
    print "PRK " + ":".join("{0:x}".format(ord(c)) for c in PRK)
    print "--"
    # expand
    blocks = []
    counter = 1
    ...
    while hlen*len(blocks) < dkLen:
        tmp = t + CTXinfo + chr(counter) # t+CTXinfo+chr(counter)
        #tmp = CTXinfo+chr(counter)
        print len(tmp)
        print " - eh?? " + ":".join("{0:x}".format(ord(c)) for c in tmp)
        t = hmac.new(PRK, tmp, digest).digest()
        print "next: " + ":".join("{0:x}".format(ord(c)) for c in t)
        blocks.append(t)
        counter += 1
